...
namespace PLAYERTWO.PlatformerProject
{
	public abstract class Entity : MonoBehaviour
	{
		...
		protected Collider[] m_contacts = new Collider[10];
		protected Collider[] m_penetrations = new Collider[5];
	...
	{
		protected IEntityContact m_contact;
		...
		{
			var overlaps = OverlapEntity(m_contacts);

			for (int i = 0; i < overlaps; i++)
			{
				if (!m_contacts[i].isTrigger && m_contacts[i].transform != transform)
				{
					OnContact(m_contacts[i]);

					if (m_contacts[i].TryGetComponent(out m_contact))
					{
						m_contact.OnEntityContact((T)this);
					}

					if (m_contacts[i].bounds.min.y > position.y)
					{
		...
		protected virtual void HandlePenetration()
		{
			...
			{
				var overlaps = OverlapEntity(m_penetrations, m_penetrationOffset);

				for (int i = 0; i < overlaps; i++)
				{
					if (!m_penetrations[i].isTrigger && m_penetrations[i].transform != transform)
					{
						if (Physics.ComputePenetration(controller, position, transform.rotation,
							m_penetrations[i], m_penetrations[i].transform.position,
							m_penetrations[i].transform.rotation, out var direction, out float distance))
						{
